		...
		methods: {
			...
			printPurchase: function() {
				purchaseService.get.getPurchaseOrderDetail(vm.request, {}, function(responseDate) {
					...
					printStr+=enter+"预付金额:"+responseDate.data.advanceMoney;
					printStr+=enter+"采购总数:"+responseDate.data.detailSummary.purchaseAmount;
					printStr+=enter+"采购总额:"+responseDate.data.detailSummary.purchaseMoney;
					...
					_.each(responseDate.data.detailList,function(item){
						...
						printStr+=enter+"采购价:"+item.purchasePrice+"  数量:"+item.purchaseAmount;
						printStr+=enter+"税率:"+item.taxRate+"  税额:"+item.taxMoney;
						printStr+=enter+"折扣:"+item.discountRate+"   总金额:"+(item.purchaseMoney-item.taxMoney);
						printStr+=enter+"-----------------------";
			...
			printReceive: function() {
				purchaseService.get.getReceiveOrderDetail(vm.request, {}, function(responseDate) {
					...
					printStr+=enter+"付款金额:"+responseDate.data.advanceMoney;
					printStr+=enter+"经手人:"+responseDate.data.sponsorName+enter+enter;
					...
					_.each(responseDate.data.detailList,function(item){
						printStr+=enter+"名称:"+item.kindName;
						printStr+=enter+"颜色:"+item.colorName+"  尺码:"+item.sizeText;
						printStr+=enter+"收货价:"+item.receivePrice+"  数量:"+item.receiveAmount;
						printStr+=enter+"税率:"+item.taxRate+"  税额:"+item.taxMoney;
						printStr+=enter+"折扣:"+item.discountRate+"  总金额:"+(item.receiveMoney-item.taxMoney);
						printStr+=enter+"-----------------------";
			...
			printReturn: function() {
				purchaseService.get.getReturnOrderDetail(vm.request, {}, function(responseDate) {
					...
					_.each(responseDate.data.detailList,function(item){
						printStr+=enter+"名称:"+item.kindName;
						printStr+=enter+"颜色:"+item.colorName+"  尺码:"+item.sizeText;
						printStr+=enter+"退货价:"+item.returnPrice+"  数量:"+item.returnAmount;
						printStr+=enter+"税率:"+item.taxRate+"  税额:"+item.taxMoney;
						printStr+=enter+"折扣:"+item.discountRate+"   总金额:"+(item.returnMoney-item.taxMoney);
						printStr+=enter+"-----------------------";
			...
			printPay: function() {
				purchaseService.get.getPayOrderDetail(vm.request, {}, function(responseDate) {
					...
					_.each(responseDate.data.sourceDetailList,function(item){
						...
						printStr+=enter+"单据金额:"+item.sumMoney+"  本次付款额:"+item.payMoney;
					...
					_.each(responseDate.data.detailList, function(item) {
						...
						printStr += enter + "付款金额:" + item.payMoney + "  手续费:" + item.feeMoney;
						printStr += enter + "支出金额:" + item.accountingMoney + "  备注:" + item.description;
						printStr += enter + "-----------------------";
